...
/*   Updated: 2025/06/28 19:42:27 by mmonika          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_col	parse_color(char *tokens)
{
	...
	{
		ft_putstr_fd("Error\ninvalid RGB value", STDERR_FILENO);
		free_array(rgb);
		exit(1);
	}
	free_array(rgb);
...
void	parse_camera(char **tokens, t_mrt *mrt)
{
	...
	{
		ft_putstr_fd("Error\ncamera Norm out of range", STDERR_FILENO);
		exit(1);
	}
...
void	parse_light(char **tokens, t_mrt *mrt)
{
	...
	{
		ft_putstr_fd("Error\nlight's Brightness out of range", STDERR_FILENO);
		exit(1);
	}
